...
One other source of incompatibility that is usually solvable, is that some software is very picky about which C or C++ compiler it uses. By default versions of Xcode since 4.2 no longer ship a standard gcc. By default they provide llvm-gcc (gcc front-end with llvm backend) and clang (clang front-end with llvm backend). Most well-written software works just fine with with clang, and most software written specifically for gcc will work well with llvm-gcc. The occasional old software will include some weird gcc-specific code. `brew install gcc` can solve many of these problems. Additionally, the homebrew/versions repository contains the last 6 versions of GCC, and Homebrew Formula can specify which compiler if Clang is not compatible. You can either directly force compilation in the formula by setting the 'CC' environment variable, or use the HOMEBREW_CC environment variable to specify a certain C compiler.
